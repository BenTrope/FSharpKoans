...
module ``08: Putting the Function into Functional Programming`` = 
    ...
    let ``04 The input to a function is a pattern (Part 3).`` () =
        ...
        let book = { Title="Theaetetus"; Author="Plato"; Year= -360 }
        (fun { Author=k } -> "Author is " + k) book |> should equal "Author is Plato"   //Theaetetus satisfies #ironic_wednesdays
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun a -> fun b -> 10 - 9) 10 9 |> should equal 1

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun a b -> 10 - 9) 10 9 |> should equal 1
    ...
    let ``19 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows h = (f "cow" h ) + ", de gozru"
                   // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
        cows "moo" |> should equal "cow says moo, de gozru"
        cows "MOOooOO" |> should equal "cow says MOOooOO, de gozru"
    ...
    let ``21 Getting closure`` () =
        ...
        // note the number of inputs provided below.  Do you see why I can do this?                                                            ////////////////////////////////// NOT REALLY
        calculate 10 20 5 |> should equal (0,10)                                                                                                // TUPLE
